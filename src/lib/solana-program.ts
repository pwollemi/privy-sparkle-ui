...
export const useSolanaProgram = () => {
  ...
  const createTokenPool = async (params: TokenCreationParams): Promise<PoolCreationResult> => {
    ...
    try {
      ...
      const configKey = new PublicKey("6RJzkvmtDyZNjhQZtpHtywG6yxpYQJLdTPNw5EpaZ3Z5");
      ...
      const poolDetails = created
        ? {
            ...
            creator: acc?.poolCreator?.toBase58?.() ?? acc?.creator?.toBase58?.() ?? acc?.creator?.toString?.(),
...
const updateUserTokenBalance = async (userWallet: string, tokenMint: string): Promise<void> => {
  try {
    ...
    const { data, error } = await supabase.from("token_balances").upsert(
      {
        user_wallet: userWallet,
        token_mint: tokenMint,
        balance: balance,
